...
/** Every dated book payment of one project's customers as flow rows for the
 *  project page's money-flow chart (same shape as the Dashboard's): deposits
 *  + profit credits are "+", withdrawals "−". All time, so the card's date
 *  filter (presets / custom) works client-side; a project holds at most a
 *  few thousand rows. Chunked .in() + paged past the 1000-row cap. */
export async function hubProjectFlowTxns(customerIds: string[]): Promise<{ date: string; op: string; amount: number }[]> {
  const admin = getAdmin();
  const out: { date: string; op: string; amount: number }[] = [];
  if (!admin || !customerIds.length) return out;
  for (let c = 0; c < customerIds.length; c += 200) {
    ...
    for (let from = 0; ; from += 1000) {
      ...
      for (const r of rows) {
        ...
        if (!r.date || !amt) continue;
        out.push({ date: r.date, op: r.kind === "withdrawal" ? "-" : "+", amount: amt });
      }
