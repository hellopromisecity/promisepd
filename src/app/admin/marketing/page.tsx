...
export default async function MarketingOverviewPage() {
  ...
  const [officersRes, itemsRes, entriesRes] = await Promise.all([
    admin
      ...
      .select("id, name, officer_type, position, district, officer_code, mobile, points, afr_total")
      .order("points", { ascending: false }),
    admin
      .from("marketing_point_items")
      .select("id, label, points, afr")
      .eq("active", true)
      .order("sort", { ascending: true })
      .order("created_at", { ascending: true }),
    admin
      .from("marketing_point_entries")
      .select("officer_id, points, afr, created_at")
  ...
  const officers: Officer[] = (officersRes.data ?? []).map((o) => ({
    ...
    points: Number(o.points) || 0, afr: Number(o.afr_total) || 0,
  }));
  const items: PointItem[] = (itemsRes.data ?? []).map((i) => ({
    id: i.id, label: i.label, points: Number(i.points) || 0, afr: Number(i.afr) || 0,
  }));
  const entries: Entry[] = (entriesRes.data ?? []).map((e) => ({
    officer_id: e.officer_id, points: Number(e.points) || 0, afr: Number(e.afr) || 0, created_at: e.created_at,
  }));
